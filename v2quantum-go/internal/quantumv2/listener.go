...
func Listen(address string, idleTimeout time.Duration, settings Settings) (*Listener, error) {
	pc, err := net.ListenPacket("udp", address)
	if err != nil {
		return nil, err
	}
	return ListenPacket(pc, idleTimeout, settings)
}

func tunePacketBuffers(pc net.PacketConn, size int) {
	type bufferTuner interface {
		SetReadBuffer(int) error
		SetWriteBuffer(int) error
	}
	if tuned, ok := pc.(bufferTuner); ok {
		_ = tuned.SetReadBuffer(size)
		_ = tuned.SetWriteBuffer(size)
	}
}

func ListenPacket(pc net.PacketConn, idleTimeout time.Duration, settings Settings) (*Listener, error) {
...
func (l *Listener) handleConnect(remote net.Addr, p packet) {
	...
	if conn == nil {
		id := p.sessionID
		conn = newConn(l.pc, remote, id, l.idleTimeout, l.settings, false, func() {
			l.mu.Lock()
			delete(l.conns, id)
			l.mu.Unlock()
		})
