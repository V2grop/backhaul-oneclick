...
# Backhaul v2 Manager - English v2.2.0
...
require_commands() {
    local cmd missing=0
    for cmd in curl systemctl timeout awk sed grep mktemp install od sysctl; do
...
install_iptables() {
    ...
    warn "iptables is not installed. Installing it now..."
    if command -v apt-get >/dev/null 2>&1; then
        ...
        error "No supported package manager was found. Install iptables manually."
        return 1
    fi

    command -v iptables >/dev/null 2>&1 || {
        error "iptables installation failed."
        return 1
    }
    ok "iptables is installed."
}

ensure_forwarding_prerequisites() {
    local tun_name="$1"
    local sysctl_file="/etc/sysctl.d/99-backhaul-v2-forwarding.conf"
    local iptables_bin

    install_iptables || return 1
    ...
    if ! sysctl -p "$sysctl_file" >/dev/null; then
        error "Failed to apply kernel forwarding settings."
        return 1
    fi

    iptables_bin="$(command -v iptables)"
    "$iptables_bin" -w 5 -C INPUT -i "$tun_name" -j ACCEPT 2>/dev/null ||
        "$iptables_bin" -w 5 -I INPUT 1 -i "$tun_name" -j ACCEPT
    "$iptables_bin" -w 5 -C OUTPUT -o "$tun_name" -j ACCEPT 2>/dev/null ||
        "$iptables_bin" -w 5 -I OUTPUT 1 -o "$tun_name" -j ACCEPT
    "$iptables_bin" -w 5 -C FORWARD -i "$tun_name" -j ACCEPT 2>/dev/null ||
        "$iptables_bin" -w 5 -I FORWARD 1 -i "$tun_name" -j ACCEPT
    "$iptables_bin" -w 5 -C FORWARD -o "$tun_name" -j ACCEPT 2>/dev/null ||
        "$iptables_bin" -w 5 -I FORWARD 1 -o "$tun_name" -j ACCEPT

    ok "Kernel forwarding and firewall rules are ready for ${tun_name}."
}
...
ask_bool() {
    ...
    while true; do
        read -r -p "$label [y/n, default: $default]: " answer
...
ask_endpoint() {
    ...
    while true; do
        ...
        error "Valid format: IP:PORT, DOMAIN:PORT, or [IPv6]:PORT"
    done
}
...
ask_bind() {
    ...
    while true; do
        ask value "Bind address or tunnel port" "8443"
        if valid_bind "$value"; then
            [[ "$value" =~ ^[0-9]+$ ]] && value=":$value"
            printf -v "$variable" '%s' "$value"
            return 0
        fi
        error "Valid examples: 8443, :8443, or 0.0.0.0:8443"
    done
}
...
ask_cidr() {
    ...
    while true; do
        ...
        error "Enter a valid CIDR, for example: 10.10.10.1/24"
    done
}
...
ask_port_mappings() {
    ...
    printf '%s\n' "${CYAN}Formats:${RESET} 443 | 443=5000 | 443-600 | 443-600:5201"
    while true; do
        ask value "Comma-separated port mappings, for example 443,2053=443"
        value="$(trim_spaces "$value")"
        [[ -n "$value" ]] || {
            error "Enter at least one port mapping."
            continue
        }
        ...
        for item in "${items[@]}"; do
            if ! validate_mapping_item "$item"; then
                error "Invalid port mapping: $item"
                continue 2
            fi
        done
...
core_version() {
    if [[ -x "$CORE_BIN" ]]; then
        "$CORE_BIN" -v 2>/dev/null || printf 'unknown'
    else
...
download_core() {
    ...
    if [[ "$(uname -m)" != "x86_64" ]]; then
        error "${CORE_ASSET} in the repository is built for x86_64; current architecture: $(uname -m)"
        return 1
    fi
    ...
    info "Downloading the core from your GitHub repository..."
    ...
    if [[ "$magic" != "7f454c46" ]]; then
        error "The downloaded file is not a valid ELF binary. Nothing was replaced."
        rm -f "$tmp"
        return 1
    fi
    ...
    version="$("$tmp" -v 2>/dev/null)" || {
        error "The downloaded core could not run. Nothing was replaced."
        rm -f "$tmp"
        return 1
    }
    if [[ "$version" != v2.* ]]; then
        error "The downloaded core is not v2: $version"
        rm -f "$tmp"
        return 1
    fi
    ...
    if [[ -e "$CORE_BIN" ]]; then
        ...
        info "Previous core backed up to: ${CORE_BIN}.backup-${stamp}"
    fi

    install -m 0755 "$tmp" "$CORE_BIN"
    rm -f "$tmp"
    ok "Core installed: $version"
    ...
    if [[ "$restart_services" == "true" ]]; then
        ...
        ((${#active_services[@]} > 0)) && ok "Backhaul v2 services restarted."
    fi
}

ensure_core() {
    if [[ ! -x "$CORE_BIN" ]]; then
        warn "The v2 core was not found at $CORE_BIN."
...
choose_role() {
    printf '\n%sServer role:%s\n' "$BOLD" "$RESET"
    printf '  %s1)%s IRAN (Server / Listener)\n' "$GREEN" "$RESET"
    printf '  %s2)%s KHAREJ (Client / Dialer)\n' "$MAGENTA" "$RESET"
    while true; do
...
choose_transport() {
    ...
    while true; do
        read -r -p "Select [1-${#transports[@]}]: " choice
        if [[ "$choice" =~ ^[0-9]+$ ]] &&
...
prompt_transport_settings() {
    ...
    if [[ "$cfg_tun_encapsulation" == "ipx" ]]; then
        ...
        ask_bool cfg_nodelay "Enable TCP_NODELAY?" "y"
    fi

    if [[ "$cfg_role" == "server" ]]; then
        if [[ "$cfg_transport" == "tcp" && "$cfg_tun_encapsulation" != "ipx" ]]; then
            ask_bool cfg_accept_udp "Accept UDP over TCP?" "n"
        fi
        if [[ "$cfg_tun_encapsulation" == "ipx" ]]; then
            cfg_proxy_protocol=""
        else
            case "$cfg_transport" in
                ...
                *) ask_bool cfg_proxy_protocol "Enable Proxy Protocol?" "n" ;;
            esac
        fi
    elif [[ "$cfg_transport" != "tun" && "$cfg_tun_encapsulation" != "ipx" ]]; then
        ask_port cfg_connection_pool "Connection Pool" "8"
    fi
}
...
prompt_connection_settings() {
    ...
    printf '\n%sConnection Configuration%s\n' "$BLUE" "$RESET"
    if [[ "$cfg_role" == "server" ]]; then
        ask_bind cfg_bind_addr
    else
        ask_endpoint cfg_remote_addr "IRAN server address"
        case "$cfg_transport" in
            ws|wss|wsmux|wssmux|xwsmux)
                ask cfg_edge_ip "Edge IP/Domain (optional)"
                ;;
        esac
    fi
}
...
prompt_tun_settings() {
    ...
    while true; do
        ...
        case "$choice" in
            ...
            *) error "Select either tcp or ipx." ;;
        esac
    done

    ask cfg_tun_name "TUN device name" "backhaul"
    if [[ ! "$cfg_tun_name" =~ ^[A-Za-z0-9_.-]+$ ]]; then
        error "Invalid TUN device name."
        return 1
    fi
    ...
    ask_cidr cfg_tun_local_addr "TUN local address" "$default_local"
    ask_cidr cfg_tun_remote_addr "TUN remote address" "$default_remote"
    ...
    [[ "$cfg_tun_mtu" =~ ^[0-9]+$ ]] &&
        ...
        error "MTU must be between 576 and 9000."
        return 1
    }
}
...
prompt_ipx_settings() {
    ...
    while true; do
        read -r -p "Profile [1-6, default: tcp]: " choice
        choice="${choice:-tcp}"
        case "$choice" in
    ...
    while true; do
        ask cfg_ipx_listen_ip "Listen IP of this server" "$default_ip"
        [[ -n "$cfg_ipx_listen_ip" ]] && break
        error "Listen IP cannot be empty."
    done
    while true; do
        ask cfg_ipx_dst_ip "Peer server IP (Destination IP)"
        [[ -n "$cfg_ipx_dst_ip" ]] && break
        error "Destination IP cannot be empty."
    done
    ...
    while true; do
        ask cfg_ipx_interface "Network interface" "$default_interface"
        [[ "$cfg_ipx_interface" =~ ^[A-Za-z0-9_.:-]+$ ]] && break
        error "Invalid network interface name, for example: eth0"
    done
    ...
    if [[ "$cfg_ipx_profile" == "icmp" ]]; then
        ...
        [[ "$cfg_ipx_icmp_type" =~ ^[0-9]+$ ]] &&
            ...
            error "ICMP Type must be between 0 and 255."
            return 1
        }
        [[ "$cfg_ipx_icmp_code" =~ ^[0-9]+$ ]] &&
            (( cfg_ipx_icmp_code >= 0 && cfg_ipx_icmp_code <= 255 )) || {
            error "ICMP Code must be between 0 and 255."
            return 1
        }
    fi
}
...
prompt_security_settings() {
    printf '\n%sSecurity Configuration%s\n' "$BLUE" "$RESET"

    if [[ "$cfg_tun_encapsulation" == "ipx" ]]; then
        read -r -p "IPX PSK (press Enter to disable encryption): " cfg_psk
        if [[ -z "$cfg_psk" ]]; then
            cfg_enable_encryption="false"
            cfg_algorithm=""
            cfg_kdf_iterations=""
            info "IPX encryption disabled; no PSK is required."
            cfg_token=""
            return 0
        fi

        cfg_enable_encryption="true"
        printf 'Algorithms: aes-256-gcm | chacha20-poly1305 | aes-128-gcm\n'
        while true; do
        ...
        [[ "$cfg_kdf_iterations" =~ ^[0-9]+$ ]] &&
            ...
            error "KDF Iterations must be a positive number."
            return 1
        }
        cfg_token=""
        return 0
    ...
    while true; do
        ask cfg_token "Shared security token"
        [[ -n "$cfg_token" ]] && return 0
        error "Security token cannot be empty."
    done
}

prompt_mux_settings() {
    [[ "$cfg_transport" == *mux ]] || return 0
    printf '\n%sMux Configuration%s\n' "$BLUE" "$RESET"
    ask cfg_mux_version "Mux Version" "2"
    [[ "$cfg_mux_version" == "1" || "$cfg_mux_version" == "2" ]] || {
        error "Mux Version must be 1 or 2."
        return 1
    }
...
prompt_tls_settings() {
    ...
    printf '\n%sTLS Configuration%s\n' "$BLUE" "$RESET"
    if [[ "$cfg_role" == "client" ]]; then
        ask cfg_tls_sni "SNI domain" "www.digikala.com"
        return 0
    fi

    mkdir -p "$CERT_DIR"
    if [[ ! -s "$CERT_FILE" || ! -s "$KEY_FILE" ]]; then
        ask_bool generate "Generate a self-signed certificate?" "y"
        if [[ "$generate" == "true" ]]; then
            ...
            openssl req -newkey ec \
                ...
                error "Failed to generate the TLS certificate."
                return 1
            }
            chmod 0600 "$KEY_FILE"
    ...
    ask cfg_tls_cert "Certificate path" "$CERT_FILE"
    ask cfg_tls_key "Private key path" "$KEY_FILE"
    [[ -s "$cfg_tls_cert" && -s "$cfg_tls_key" ]] || {
        error "Certificate or private key file was not found."
        return 1
    }
}
...
prompt_tuning_settings() {
    ...
    printf '\n%sTuning Configuration%s\n' "$BLUE" "$RESET"
    ask_bool advanced "Change advanced tuning settings?" "n"
    [[ "$advanced" == "true" ]] || {
    ...
    ask_bool cfg_auto_tuning "Enable Auto Tuning?" "y"
    ask cfg_tuning_profile "Kernel profile (balanced/fast/latency/resource)" "balanced"
    case "$cfg_tuning_profile" in
        balanced|fast|latency|resource) ;;
        *) error "Invalid kernel profile."; return 1 ;;
    esac

    ask cfg_workers "Workers (0 = auto)" "0"
    [[ "$cfg_workers" =~ ^[0-9]+$ ]] || {
    ...
    [[ "$cfg_channel_size" =~ ^[0-9]+$ ]] || {
        error "Channel Size must be a number."
        return 1
    }

    if [[ "$cfg_tun_encapsulation" == "ipx" ]]; then
        ask cfg_batch_size "Batch Size" "2048"
        ask cfg_so_sndbuf "SO_SNDBUF (0 = auto)" "0"
        [[ "$cfg_batch_size" =~ ^[0-9]+$ && "$cfg_so_sndbuf" =~ ^[0-9]+$ ]] || {
            error "Batch Size and SO_SNDBUF must be numbers."
            return 1
        }
        cfg_so_rcvbuf=""
        cfg_tcp_mss=""
        ...
        case "$cfg_buffer_profile" in
            ...
            *) error "Invalid buffer profile."; return 1 ;;
        esac
    fi
}
...
prompt_logging_settings() {
    ...
    case "$cfg_log_level" in
        ...
        *) error "Invalid log level."; return 1 ;;
    esac
}
...
prompt_ports_settings() {
    ...
    printf '\n%sPort Mapping Configuration%s\n' "$BLUE" "$RESET"
    if [[ "$cfg_transport" == "tun" ]]; then
        ask cfg_forwarder "Forwarder (backhaul/iptables)" "iptables"
        case "$cfg_forwarder" in
            backhaul|iptables) ;;
            *) error "Invalid forwarder."; return 1 ;;
        esac
    fi
...
validate_generated_config() {
    ...
    if config_has_parser_error "$log"; then
        error "The core rejected the generated configuration:"
        sed -n '1,30p' "$log" >&2
        rm -f "$log"
        return 1
    fi
    ...
    if (( rc != 0 && rc != 124 )); then
        warn "The configuration syntax is valid; runtime status will be checked after service startup."
    fi
    rm -f "$log"
}
...
create_service() {
    ...
    local port="$2"
    local config="$3"
    local service service_file iptables_bin sysctl_bin
...

    {
        ...
        printf 'Type=simple\n'
        printf 'User=root\n'
        if [[ "$cfg_transport" == "tun" && -n "$iptables_bin" ]]; then
            printf 'ExecStartPre=%s -w net.ipv4.ip_forward=1\n' "$sysctl_bin"
            printf 'ExecStartPre=%s -w net.ipv6.conf.all.forwarding=1\n' "$sysctl_bin"
            printf 'ExecStartPre=%s -w net.ipv4.conf.all.rp_filter=0\n' "$sysctl_bin"
            printf 'ExecStartPre=%s -w net.ipv4.conf.default.rp_filter=0\n' "$sysctl_bin"
            printf 'ExecStartPre=/bin/sh -c "%s -w 5 -C INPUT -i %s -j ACCEPT 2>/dev/null || %s -w 5 -I INPUT 1 -i %s -j ACCEPT"\n' \
                "$iptables_bin" "$cfg_tun_name" "$iptables_bin" "$cfg_tun_name"
            printf 'ExecStartPre=/bin/sh -c "%s -w 5 -C OUTPUT -o %s -j ACCEPT 2>/dev/null || %s -w 5 -I OUTPUT 1 -o %s -j ACCEPT"\n' \
                "$iptables_bin" "$cfg_tun_name" "$iptables_bin" "$cfg_tun_name"
            printf 'ExecStartPre=/bin/sh -c "%s -w 5 -C FORWARD -i %s -j ACCEPT 2>/dev/null || %s -w 5 -I FORWARD 1 -i %s -j ACCEPT"\n' \
                "$iptables_bin" "$cfg_tun_name" "$iptables_bin" "$cfg_tun_name"
            printf 'ExecStartPre=/bin/sh -c "%s -w 5 -C FORWARD -o %s -j ACCEPT 2>/dev/null || %s -w 5 -I FORWARD 1 -o %s -j ACCEPT"\n' \
                "$iptables_bin" "$cfg_tun_name" "$iptables_bin" "$cfg_tun_name"
        fi
    ...
    if systemctl is-active --quiet "$service"; then
        ok "Tunnel service is active: $service"
        return 0
    fi

    error "The service was created but failed to start: $service"
...
configure_tunnel() {
    ...
    reset_config_values
    clear
    printf '%s%sConfigure a new Backhaul v2 tunnel%s\n' "$BOLD" "$CYAN" "$RESET"
    ...
    if [[ "$cfg_transport" == "tun" ]]; then
        ensure_forwarding_prerequisites "$cfg_tun_name" || {
            pause
            return
        }
    fi
    ...
    temp_config="$(mktemp "${CORE_DIR}/.v2-config.XXXXXX")" || {
        error "Failed to create a temporary configuration file."
        pause
        return
    }
    generate_config "$temp_config"

    info "Validating the configuration with $(core_version)..."
...
status_label() {
    ...
    if systemctl is-active --quiet "$service"; then
        ...
        printf '%sinactive/failed%s' "$RED" "$RESET"
    fi
}
...
show_all_statuses() {
    ...
    clear
    printf '%s%sBackhaul v2 tunnel status%s\n\n' "$BOLD" "$CYAN" "$RESET"
    ...
    (( found == 1 )) || warn "No Backhaul v2 configuration was found."
    pause
}
...
select_config() {
    ...
    if ((${#configs[@]} == 0)); then
        warn "No Backhaul v2 tunnel exists."
        return 1
    fi
...
remove_selected_tunnel() {
    ...
    service="$(config_service_name "$config")"
    service_file="${SERVICE_DIR}/${service}"
    read -r -p "Remove ${service}? Type DELETE to confirm: " confirm
    [[ "$confirm" == "DELETE" ]] || {
        warn "Cancelled."
        return 0
    }
    ...
    ok "Tunnel removed. Recovery copy: $backup_dir"
}
...
manage_tunnel() {
    ...
    while true; do
        ...
        printf '  1) Restart\n'
        printf '  2) Status\n'
        printf '  3) Logs (last 100 lines)\n'
        printf '  4) Show configuration\n'
        printf '  %s5) Remove tunnel%s\n' "$RED" "$RESET"
        printf '  0) Back\n'
        read -r -p "Select: " choice
        ...
        case "$choice" in
            1)
                systemctl restart "$service" &&
                    ok "Service restarted." || error "Service restart failed."
                pause
                ;;
            2)
...
diagnostics() {
    ...
    clear
    printf '%s%sBackhaul v2 diagnostics%s\n\n' "$BOLD" "$CYAN" "$RESET"
    printf 'Core path: %s\n' "$CORE_BIN"
...
remove_core() {
    ...
    if config="$(list_configs)" && [[ -n "$config" ]]; then
        error "Remove all v2 tunnels from Tunnel Management first."
        pause
        return
    fi

    read -r -p "Remove the v2 core file? Type REMOVE to confirm: " confirm
    [[ "$confirm" == "REMOVE" ]] || {
        warn "Cancelled."
        pause
        return
    ...
    if [[ -e "$CORE_BIN" ]]; then
        ...
        rm -f "$CORE_BIN"
        ok "Core removed; a recovery copy was kept beside it."
    else
        warn "Core was not installed."
    fi
    pause
}
...
main_menu() {
    ...
    while true; do
        show_header
        printf '  %s1)%s Configure a new tunnel\n' "$GREEN" "$RESET"
        printf '  %s2)%s Tunnel management\n' "$MAGENTA" "$RESET"
        printf '  %s3)%s Check all tunnel statuses\n' "$CYAN" "$RESET"
        printf '  4) Install/update backhaul_premium_v2\n'
        printf '  5) Diagnostics\n'
        printf '  %s6)%s Remove the v2 core\n' "$RED" "$RESET"
        printf '  0) Exit\n\n'
...
trap 'printf "\n"; warn "Operation interrupted."; exit 130' INT TERM
require_root
require_commands
