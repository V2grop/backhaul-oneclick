...
install_watchdog() {
  ...
  if [[ "$WATCHDOG" != "true" ]]; then
    ...
    rm -f -- "$watchdog_script" "$watchdog_service" "$watchdog_timer" \
      "/run/backhaul-max/${SERVICE}.service.failures" \
      "/run/backhaul-max/${SERVICE}.service.last-restart"
...
STATE_DIR="\${BACKHAUL_WATCHDOG_STATE_DIR:-/run/backhaul-max}"
STATE_FILE="\${STATE_DIR}/${SERVICE}.failures"
RESTART_FILE="\${STATE_DIR}/${SERVICE}.last-restart"
FAILURE_THRESHOLD=2
RESTART_COOLDOWN=45
mkdir -p "\$STATE_DIR"
...
logger -t backhaul-max-watchdog "Restarting \$SERVICE after \$failures failed transport checks"
printf '%s\n' "\$now" > "\$RESTART_FILE"
systemctl restart "\$SERVICE" 2>/dev/null || true
sleep 3
if ! systemctl is-active --quiet "\$SERVICE"; then
  logger -t backhaul-max-watchdog "Soft restart failed for \$SERVICE; forcing a clean start"
...
[Unit]
Description=Check ${SERVICE} XWSMUX transport every 15 seconds
...
install_current_tunnel() {
  ...
  local had_config=false had_unit=false was_active=false replace_existing
  role_paths

  if [[ -e "$CONFIG" || -e "$UNIT" ]]; then
    prompt_yes_no "Tunnel ${SERVICE} already exists. Replace only this instance?" "false" replace_existing
    [[ "$replace_existing" == "true" ]] || {
      warn "Nothing changed. Use another tunnel port to create an additional tunnel."
      return 0
    }
  fi
...
remove_tunnel() {
  ...
  rm -f -- "${SERVICE_DIR}/${SEL_SERVICE}-watchdog.service" \
    ...
    "/run/backhaul-max/${SEL_SERVICE}.service.failures" \
    "/run/backhaul-max/${SEL_SERVICE}.service.last-restart"
